...
import { orm, connect, loadMigration, fixtures, section, eq, rejects, done } from "./_harness.mjs";
...
await rejects(
  ...
  () => initDatabase({ migrations: `${fixtures}migrations-rollback`, seeds: `${fixtures}seeds-empty` }),
  /already exists/,
);
...
const opts = { migrations: `${fixtures}migrations-race`, seeds: `${fixtures}seeds-empty` };
